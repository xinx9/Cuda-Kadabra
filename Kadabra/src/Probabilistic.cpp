...
///////////////////////////////////////////////////////////////////////////////////////////////////
/*
__global__ void kernelInitArray
(uint64_t n_pair, uint64_t vis_edges, vector<uint32_t> path, double *approx, uint32_t *top_k)
{
...
void Probabilistic::one_round(Sp_sampler &sp_sampler) {
    ...
    //////////////////////////////
/*    cudaMalloc((void**)&top_k, omp_get_thread_num()*sizeof(top_k));

    dim3 dimBlock(1024,1,1);
    dim3 dimGrid((omp_get_thread_num() + threadsPerBlock.x-1)/threadsPerBlock.x,1,1);
    
    thrust::host_vector<int> hostCounts(omp_get_thread_num(), 0);
    thrust::device_vector<int> deviceCounts(hostCounts);
    int * dcountsptr = thrust::raw_pointer_cast(&deviceCounts[0]);

    kernelInitArray<<<dimGrid,dimBlock>>>(n_pair,vis_edges,path,approx,top_k);
  */  
    //////////////////////
    //	kernel stop	//
    //////////////////////

//cout << "\nC0 one_round threads = " << omp_get_thread_num() << endl; 


   
    #pragma omp critical
    {
	cout << "\nC1 one_round threads = " << omp_get_thread_num() << endl;
        n_pairs++;
        vis_edges += sp_sampler.vis_edges;
...
cout << "time to complete: " << timevar.count() << " seconds" << endl;
    }
...
void Probabilistic::get_status (Status *status) const {
    time_critical[omp_get_thread_num()] -= get_time_sec();
//    cout << "\nC0 get_status threads = " << omp_get_thread_num() << endl;
    #pragma omp critical
    {
//	cout << "\nC1 get_status threads = " << omp_get_thread_num() << endl;
        if (status != NULL) {
...
//    cout << "\nC0:0 run threads = " << omp_get_thread_num() << endl;
    #pragma omp parallel
    {
//        cout << "\nC1:0 run threads = " << omp_get_thread_num() << endl;
        ...
        while (n_pairs <= tau) {
            ...
            if (verbose > 0 && current_time - last_output > verbose) {
                #pragma omp critical
                {
//	            cout << "\nC2:0 run threads = " << omp_get_thread_num() << endl;
...
//    cout << "\nC0:1 run threads = " << omp_get_thread_num() << endl;
    #pragma omp parallel
    {
//	cout << "\nC1:1 run threads = " << omp_get_thread_num() << endl;
        Sp_sampler sp_sampler( this, random_seed[omp_get_thread_num()] );
        Status status(union_sample);
        status.n_pairs = 0;
        bool stop = false;
        ...
        while( !stop && status.n_pairs < omega ) {
            ...
            if (verbose > 0 && current_time - last_output > verbose) {
                ...
                {
//		    cout << "\nC2:1 run threads = " << omp_get_thread_num() << endl;
                    if (current_time - last_output > verbose) {
                        last_output = current_time;
                        print_status(&status);
                    }
